...
# Amazon = nx.read_gpickle('../Freebase.gpickle')

for i in range(0,20):
    print("\n-----------------------------")
    print("Now processing the " + str(i) + "th motif")
    ...
    target_vertex_id = random.sample(q.nodes(), 1)[0]

    # target_vertex_id = 7239
    quality_limitation = 0

    # q = nx.read_gpickle('../toy_motif_of_DBLP.gpickle')
    # target_vertex_id = 16096; target_type = "paper"

    # nx.write_gpickle(q,'../toy_motif_of_DBLP.gpickle')
    ...
    nx.write_gpickle(EFS_graph,'../toy_EFS_DBLP_Graph.gpickle')
    # #
    #
    ...
    # start = time.time()
    # S, D = generate_dictionary(q, Amazon, target_vertex_id)
