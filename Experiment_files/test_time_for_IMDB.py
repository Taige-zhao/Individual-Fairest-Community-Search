...
for i in range(0, number_of_motifs):
    ...
    print("Find motifs")
    # q, target_vertex_id, _ = select_motif(DBLP, 7)
    q = nx.DiGraph()
    ...
    q.add_edge(4, 2)
    target_vertex_id = 0
    print("Finish find motif")

    print("Start adv")
    start = time.time()
    _, _, _, _ = Adv_BaseLine(DBLP, q, target_vertex_id, 10, 0.6)
    time_list_baseline.append(time.time() - start)

    print("Finish Adv")

    _, _, time_EFS = EFS(DBLP, q, target_vertex_id, 10, 0.6)
