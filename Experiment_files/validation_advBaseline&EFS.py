...
from utils import Baseline, Adv_BaseLine, EFS
...
start = time.time()
D_, M_graph, maxC, best_score = Adv_BaseLine(DBLP, q, target_vertex_id, 10, 1)
...
start = time.time()
maxC_, best_score_ = EFS(DBLP, q, target_vertex_id, 10, 1)
print(str(time.time() - start) + "s")
#
